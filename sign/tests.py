...
class ModelTest(TestCase):
    ...
    def test_event_models(self):
        ...
        self.assertEqual(result.address,'chengdu')
...
class EventManageTest(TestCase):
    '发布会管理'
    def setUp(self):
        Event.objects.create(id=2,name='xiaomi5',limit=2000,status=True,address='beijing',start_time=datetime(2018,8,10,14,0,0))
        self.c = Client()
    ...
    def test_event_manage_search(self):
        '测试发布会搜索'
        response = self.c.post('/search_name/',{'name':'xiamomi5'})
        self.assertEqual(response.status_code,200)
        self.assertIn(b'xiaomi5',response.content)
        self.assertIn(b'beijing',response.content)
#此用例要想通过，需要在views.py视图文件中将event-manage()和search_name()函数的@login_required装饰器去掉，因为这两个函数依赖于登录，然而，Client()所提供的get()和post()方法并没有验证登录参数

'''Test Guest Manage'''
class GuestManageTest(TestCase):
    def setup(self):
        Event.objects.create(id=1,name='xiaomi5',limit=2000,address='beijing',status=1,start_time=datetime(2018,8,10,14,0,0))
...
class SignIndexActionTest(TestCase):
    def setUp(self):
        Event.objects.create(id=1,name='xiamomi5',limit=2000,address='beijing',status=1,start_time='2018-8-10 12:30:00')
        Event.objects.create(id=2,name='oneplus4',limit=2000,address='shenzhen',status=1,start_time='2018-6-10 12:30:00')
        Guest.objects.create(realname='alen',phone='18611001100',email='alen@mail.com',sign=0,event_id=1)
